...
int main(){
 try{
  ...
  for(int fi=1;fi<=12;++fi){
    ...
    pf(plan.tiles.size(),4,[&](std::size_t i,std::uint32_t){
  ...
  for(std::uint32_t workers: {1u,2u,4u}){
    ...
    std::vector<double> ms;std::vector<std::uint64_t> bytes;
    for(auto&f:frames){
      ...
      ms.push_back(std::chrono::duration<double,std::milli>(b-a).count());bytes.push_back(sz);
    }
    double med=pc(ms,.5),p95=pc(ms,.95),meanb=(double)std::accumulate(bytes.begin(),bytes.end(),(std::uint64_t)0)/bytes.size();
    std::cout<<"WORKER_SCALE_PASS workers="<<workers<<" median_ms="<<med<<" p95_ms="<<p95<<" fps="<<1000.0/med<<" mean_packed_bytes="<<meanb<<" lossless=1\n";
  }
  return 0;
