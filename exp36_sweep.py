...
ROOT=Path("silesia"); OUT=Path("exp36_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP33H_BASE",0),("EXP36A_LINE_PHASE",1),("EXP36B_PLANE_ROW",2),("EXP36C_XOR_PHASE",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir36_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp36_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
