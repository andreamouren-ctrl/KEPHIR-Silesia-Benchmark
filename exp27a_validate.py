...
ROOT=Path("silesia"); OUT=Path("exp27a_validate"); OUT.mkdir(exist_ok=True)
...
for name in FILES:
    ...
    for _ in range(3):
        ...
        t=time.perf_counter()
        subprocess.run(["./kephir27_1","cp",str(p),str(arc),"6","6.55","9.42","1.20"],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    ...
    for _ in range(3):
        ...
        t=time.perf_counter()
        subprocess.run(["./kephir27_1","dp",str(arc),str(dec),"6"],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
summary=dict(profile="KEPHIR EXP-27A TEXT-LAZY",raw=raw,size=size,ratio=100*size/raw,
             comp_MBps=(raw/1e6)/ct,dec_MBps=(raw/1e6)/dt,sha_all=True,
             params={"LIT":6.55,"MC":9.42,"DPEN_BASE":1.20,
                     "text_ge_0_75_dpen":1.02,"text_le_0_35_dpen":0.95,"mid_dpen":1.15,
                     "lazy_text_ge_0_75":28,"lazy_else":19})
print("SUMMARY",summary,flush=True)
for r in rows: print(r,flush=True)
Path("exp27a_validation.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
