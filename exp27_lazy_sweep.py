...
ROOT=Path("silesia"); OUT=Path("exp27_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP26C",0),("EXP27A_TEXT_LAZY",1),("EXP27B_BIN_LAZY",2),("EXP27C_3BAND_LAZY",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir27_{mode}"
...
Path("exp27_lazy_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
