...
Engineering milestone: chunk-local automatic selection between the current
lossless AURORA video paths. Selection is based on final EXP-37A archive bytes,
not on a proxy metric. This file is original project code.

IMPORTANT IP NOTE:
Generic per-block/per-GOP mode selection by compressed size is BACKGROUND and
is not claimed as candidate IP. It is infrastructure that lets later genuinely
KHEPRI-specific mechanisms compete fairly inside the same router.
"""
...
MAGIC=b"K5GR"
VERSION=1
HDR=struct.Struct("<4sBHHIII")  # magic,ver,w,h,fpsn,fpsd,gop
...
def decode_candidate(payload, mode, tmp, exe, w,h,idx):
    ...
    else: mc_decode(front,raw)
    fd=time.perf_counter()-t
    return raw.read_bytes(),kd+fd

def encode_file(src,dst,exe,w,h,fpsn,fpsd,gop):
    ...
    with tempfile.TemporaryDirectory(prefix="ksv05_") as td:
        tmp=Path(td)
        for gi,off in enumerate(range(0,total,gop)):
            n=min(gop,total-off)
        ...
        with dst.open("wb") as f:
            f.write(HDR.pack(MAGIC,VERSION,w,h,fpsn,fpsd,gop))
...
def decode_file(src,dst,exe):
    ...
    magic,ver,w,h,fpsn,fpsd,gop=HDR.unpack_from(b,pos); pos+=HDR.size
...
def main():
    ...
    for spec in a.clip:
        ...
        rows.append(dict(name=name,raw_bytes=src.stat().st_size,bytes=arc.stat().st_size,
                         ...
                         frames=frames,modes={"TEMP":counts[0],"MC8R4":counts[1]},
