...
ROOT=Path("silesia"); OUT=Path("exp33_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP31C_BASE",0),("EXP33A_REUSE",1),("EXP33B_LATTICE",2),("EXP33C_HYBRID",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir33_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp33_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
