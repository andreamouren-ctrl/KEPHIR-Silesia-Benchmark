...
#if defined(AURORA_DISABLE_HALFBLOCK_SAD)
    (void)stop_at;
#endif
#if defined(__SSE2__)
    std::uint64_t sum=0;
...
#if !defined(AURORA_DISABLE_HALFBLOCK_SAD)
    // The first four rows are a monotonic lower bound on the full 8x8 SAD.
    // The encoder only accepts strictly lower cost, so a candidate whose
    // half-block cost already reaches the current best cannot become winner.
    if(sum>=stop_at)
        return sum;
#endif
