...
// EXP-38A is included into this translation unit so its current static
...
#endif
#define main aurora_kephir_exp38_cli_main
#include "../../../KEPHIR_2_EXP38_FUSED_DUAL.cpp"
#undef main
#if defined(__GNUC__)
...
Bytes AuroraKhepriExp38MemoryAdapter::encode(ByteView input) {
...
Bytes AuroraKhepriExp38MemoryAdapter::decode(ByteView input) {
    if(input.size()<kHeaderSize)
        throw AuroraMediaError(ErrorCode::TruncatedInput,"KHEPRI memory frame header");
    if(input[0]!='K'||input[1]!='3'||input[2]!='8'||input[3]!='M')
