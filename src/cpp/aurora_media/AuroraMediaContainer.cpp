...
T get_le(std::istream& is) {
    ...
    for (std::size_t i=0;i<sizeof(T);++i) {
        const int c=is.get();
        if (c==EOF) throw std::runtime_error("truncated file");
...
std::array<char,4> get_magic(std::istream& is) {
    std::array<char,4> x{};
    is.read(x.data(),4);
    if(is.gcount()!=4) throw std::runtime_error("truncated magic");
    return x;
}
...
void Muxer::write_packet(std::uint8_t track_id,std::uint64_t pts,
    ...
    if(!out_) throw std::runtime_error("payload write failed");
...
void Demuxer::read_header_and_tracks() {
    if(get_magic(in_)!=kMagic) throw std::runtime_error("bad AURORA Media magic");
    ...
    for(std::uint16_t i=0;i<n;++i) {
        ...
        if(!ids.insert(t.id).second) throw std::runtime_error("duplicate track id");
        tracks_.push_back(t);
    }
...
void Demuxer::read_index() {
    ...
    if(end<kFooterSize) throw std::runtime_error("missing footer");
    in_.seekg(static_cast<std::streamoff>(end-kFooterSize));
    if(get_magic(in_)!=kFooterMagic) throw std::runtime_error("bad footer magic");
    ...
    if(static_cast<std::size_t>(in_.gcount())!=b.size()) throw std::runtime_error("truncated index");
    if(crc32(b.data(),b.size())!=expected_crc) throw std::runtime_error("index CRC");

    std::size_t p=0;
    auto get8=[&](){ if(p+1>b.size()) throw std::runtime_error("index truncated"); return b[p++]; };
...
std::vector<std::uint8_t> Demuxer::read_packet(const PacketInfo& e) {
    ...
    if(static_cast<std::size_t>(in_.gcount())!=sz && sz) throw std::runtime_error("truncated packet");
    if(crc32(p.data(),p.size())!=expected_crc) throw std::runtime_error("packet CRC");
    return p;
}
