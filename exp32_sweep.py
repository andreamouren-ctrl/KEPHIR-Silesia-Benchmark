...
ROOT=Path("silesia"); OUT=Path("exp32_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP30A_FULL",1),("EXP32A_STRIDE2",2),("EXP32B_STRIDE4",3),("EXP32C_STRIDE8",4)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir32_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
        ...
        rows.append(dict(mode=tag,file=name,raw=raw,size=size,ratio=100*size/raw,seconds=sec,sha_ok=ok))
summary=[]
for tag,mode in MODES:
...
Path("exp32_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
