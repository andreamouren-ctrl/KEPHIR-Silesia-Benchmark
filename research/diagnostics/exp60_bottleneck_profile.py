...
PROFILE_TIME=collections.Counter()

def cp(payload,tmpbase,label="normal"):
    ...
    PROFILE_TIME[label]+=time.perf_counter()-t0
    PROFILE[label]+=1
    data=arc.read_bytes()
...
def encode(src,dst,tmp):
    ...
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            comp=cp(payload,tmp/f"p{pidx}_{j}_m{mode}","primary")
            # Preserve EXP-58's BASE verification for structural transforms.
            if mode!=0:
                base_comp=cp(chunk,tmp/f"p{pidx}_{j}_basecheck","basecheck")
            ...
            if is_text_like(chunk):
                ...
                PROFILE_TIME["tokenize"]+=time.perf_counter()-t_tok
                PROFILE["tokenize_calls"]+=1
                if len(tok)+16 < len(chunk)*0.99:
                    tok_comp=cp(tok,tmp/f"p{pidx}_{j}_text","text")
...
root=Path("silesia");out=Path("exp60_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k60";dec=out/f"{fn}.dec"
...
summary=dict(
    ...
    profile_counts=dict(PROFILE),
    profile_time_s=dict(PROFILE_TIME),
    backend_cp_calls=PROFILE["primary"]+PROFILE["basecheck"]+PROFILE["text"],
    backend_cp_time_s=PROFILE_TIME["primary"]+PROFILE_TIME["basecheck"]+PROFILE_TIME["text"],
...
Path("exp60_profile_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
