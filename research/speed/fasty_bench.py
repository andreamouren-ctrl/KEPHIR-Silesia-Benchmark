...
ROOT=Path("silesia"); OUT=Path("fasty_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_V","./fast_v"),("FAST_Y","./fast_y")]
rows=[]
for tag,exe in PROFILES:
    ...
    rows.append(dict(profile=tag,raw=tr,size=ts,ratio=100*ts/tr,comp_MBps=(tr/1e6)/tc,dec_MBps=(tr/1e6)/td,sha=True))
print("SUMMARY")
for r in rows: print(r,flush=True)
if rows[0]["size"]!=rows[1]["size"]:
    raise SystemExit("FAST-Y archive size changed")
Path("fasty_results.json").write_text(json.dumps(rows,indent=2))
