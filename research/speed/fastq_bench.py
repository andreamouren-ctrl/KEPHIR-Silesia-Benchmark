...
ROOT=Path("silesia"); OUT=Path("fastq_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_K","./fast_k"),("FAST_Q","./fast_q")]
rows=[]
...
Path("fastq_results.json").write_text(json.dumps(rows,indent=2))
