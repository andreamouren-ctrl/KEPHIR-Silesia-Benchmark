...
ROOT=Path("silesia"); OUT=Path("fastj_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_D","./fast_d"),("FAST_J","./fast_j")]
rows=[]
for tag,exe in PROFILES:
    ...
    rows.append(dict(profile=tag,size=int(ts),ratio=100*ts/tr,
                     comp_MBps=(tr/1e6)/tc,dec_MBps=(tr/1e6)/td,sha=True))
print("SUMMARY")
for r in rows: print(r,flush=True)
Path("fastj_results.json").write_text(json.dumps(rows,indent=2))
