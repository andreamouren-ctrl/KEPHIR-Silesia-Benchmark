...
ROOT=Path("silesia"); OUT=Path("fastu_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_O","./fast_o"),("FAST_U","./fast_u")]
rows=[]
...
Path("fastu_results.json").write_text(json.dumps(rows,indent=2))
