...
ROOT=Path("silesia"); OUT=Path("fastm_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_K","./fast_k"),("FAST_M_PGO","./fast_m")]
rows=[]
...
Path("fastm_results.json").write_text(json.dumps(rows,indent=2))
