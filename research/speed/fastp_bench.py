...
ROOT=Path("silesia"); OUT=Path("fastp_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_K","./fast_k"),("FAST_P_HASH19","./fast_p")]
rows=[]
...
Path("fastp_results.json").write_text(json.dumps(rows,indent=2))
