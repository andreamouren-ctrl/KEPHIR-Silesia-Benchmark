...
ROOT=Path("silesia"); OUT=Path("fastl_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_K","./fast_k"),("FAST_L","./fast_l")]
rows=[]
...
Path("fastl_results.json").write_text(json.dumps(rows,indent=2))
