...
ROOT=Path("silesia"); OUT=Path("fastw_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
PROFILES=[("FAST_U","./fast_u"),("FAST_W1","./fast_w1"),("FAST_W2","./fast_w2")]
rows=[]
...
Path("fastw_results.json").write_text(json.dumps(rows,indent=2))
