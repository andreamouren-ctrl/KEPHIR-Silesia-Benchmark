...
root=Path("silesia");out=Path("exp62_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k62";dec=out/f"{fn}.dec"
...
Path("exp62_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
