...
root=Path("silesia");out=Path("exp59_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k59";dec=out/f"{fn}.dec"
...
Path("exp59_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
