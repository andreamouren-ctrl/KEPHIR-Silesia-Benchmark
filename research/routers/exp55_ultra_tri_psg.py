    ...
    for idx,start in enumerate(range(0,len(raw),CH)):
        ...
        for mode in range(6):
            ...
            for backend,exe in ((0,"./kephir37"),(1,"./kephir53"),(2,"./kephir55")):
                comp=cp(exe,payload,tmp/f"c_{idx}_{mode}_{backend}")
                # one byte descriptor already exists in container; encode backend+mode in it
                candidates.append((len(comp),backend,mode,comp))
        _,backend,mode,comp=min(candidates,key=lambda x:x[0])
        chosen[(backend,mode)]+=1
        entries.append((backend,mode,len(chunk),comp))
...
root=Path("silesia");out=Path("exp55_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k55";dec=out/f"{fn}.dec"
...
summary=dict(
    ...
    baseline_exp54_size=63200940,
    delta_bytes=size-63200940,
    improvement_bytes=63200940-size,
    improvement_pct=(63200940-size)/63200940*100.0
)
print("SUMMARY",json.dumps(summary,indent=2),flush=True)
Path("exp55_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
