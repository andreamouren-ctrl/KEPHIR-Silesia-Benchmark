...
def encode(src,dst,tmp):
    ...
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            # EXP-58 single conceptual change:
            # verify every non-BASE transform against BASE, because EXP-57
            # has enough time budget for this selective second encode.
            if mode!=0:
...
root=Path("silesia");out=Path("exp58_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k58";dec=out/f"{fn}.dec"
...
Path("exp58_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
