...
def cp(payload,tmpbase,exe="./kephir37"):
...
def dp(payload,tmpbase,exe="./kephir37"):
...
ACTIONS=("wx","psg53","psg55")

def action_key(context,action):
    return f"{context}|{action}"

def action_state(model,context,action):
    k=action_key(context,action)
    return model.setdefault(k,{"seen":0,"trials":0,"wins":0,"gain":0,"cost":0.0})

def learner_should_probe(model,context,action):
    st=action_state(model,context,action)
    st["seen"]+=1
    # Explore unknown actions first. Afterwards exploit only actions with
    # positive measured return, while keeping a sparse refresh path.
    if st["trials"]<2:return True
    if st["gain"]>0:
        avg_gain=st["gain"]/st["trials"]
        avg_cost=st["cost"]/st["trials"] if st["cost"]>0 else 1e-9
        return (avg_gain/avg_cost)>0.0
    return (st["seen"] % 32)==0

def learner_update(model,context,action,gain,cost):
    st=action_state(model,context,action)
    st["trials"]+=1
    st["gain"]+=int(gain)
    st["cost"]+=float(cost)
...
def process_parcel(args):
    ...
    tmp=Path(tmpdir)
    out=[]
    for seq,pidx,j,chunk,actions,key in parcel:
        ...
        if mode!=0:
            ...
            if len(base_comp)<=len(comp):
                mode=0;payload=chunk;comp=base_comp

        observations=[]

        # Action 1: Word-XOR candidate. It is only meaningful from BASE/non-text.
        if "wx" in actions and mode==0 and not text_like:
            t0=time.perf_counter()
            wx=transform(chunk,3)
            cand=cp(wx,tmp/f"p{pidx}_{j}_wx")
            cost=time.perf_counter()-t0
            gain=max(0,len(comp)-len(cand))
            observations.append(("wx",gain,cost))
            if len(cand)<len(comp):
                backend=0;mode=3;payload=wx;comp=cand

        # Actions 2/3: complementary PSG experts. Probe the currently retained
        # reversible representation and keep only a genuinely smaller result.
        if not text_like:
            payload=transform(chunk,mode)
            if "psg53" in actions:
                t0=time.perf_counter()
                cand=cp(payload,tmp/f"p{pidx}_{j}_psg53","./kephir53")
                cost=time.perf_counter()-t0
                gain=max(0,len(comp)-len(cand))
                observations.append(("psg53",gain,cost))
                if len(cand)<len(comp):
                    backend=1;comp=cand
            if "psg55" in actions:
                t0=time.perf_counter()
                cand=cp(payload,tmp/f"p{pidx}_{j}_psg55","./kephir55")
                cost=time.perf_counter()-t0
                gain=max(0,len(comp)-len(cand))
                observations.append(("psg55",gain,cost))
                if len(cand)<len(comp):
                    backend=2;comp=cand
        ...
        if text_like:
            ...
            if len(tok)+16 < len(chunk)*0.99:
                ...
                if len(tok_comp)<len(comp):
                    backend=0;mode=6;comp=tok_comp

        out.append((seq,backend,mode,len(chunk),comp,key,observations))
    return out
...
def encode(src,dst,tmp,model):
    ...
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            actions=[]
            # EXP-71: contextual multi-action portfolio.
            # Text keeps its proven tokenizer path; non-text can learn among
            # Word-XOR and the two complementary PSG experts.
            if not is_text_like(chunk):
                for action in ACTIONS:
                    if learner_should_probe(model,key,action):
                        actions.append(action)
            tasks.append((seq,pidx,j,chunk,tuple(actions),key))
            seq+=1
    ...
    entries=[(backend,mode,n,comp) for _,backend,mode,n,comp,_,_ in flat]
    probes=wins=gain_bytes=0;probe_cost=0.0
    action_stats=collections.defaultdict(lambda:{"probes":0,"wins":0,"gain_bytes":0,"cost_s":0.0})
    for _,_,_,_,_,key,observations in flat:
        for action,gain,cost in observations:
            probes+=1;wins+=int(gain>0);gain_bytes+=gain;probe_cost+=cost
            st=action_stats[action];st["probes"]+=1;st["wins"]+=int(gain>0);st["gain_bytes"]+=gain;st["cost_s"]+=cost
            learner_update(model,key,action,gain,cost)

    for backend,mode,n,comp in entries:
        chosen[(backend,mode)]+=1;grains[n]+=1
    ...
    return chosen,grains,workers,len(parcels),imbalance,probes,wins,gain_bytes,probe_cost,dict(action_stats)
...
out=Path("exp71_out");tmp=out/"tmp";out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
model={}
epochs=[]
for epoch in range(1,4):
    ...
    for cname,croot in CORPORA:
        ...
        for fi,src in enumerate(files):
            safe=f"{cname}_{fi}"
            arc=out/f"e{epoch}_{safe}.k71";dec=out/f"e{epoch}_{safe}.dec"
            t=time.perf_counter()
            chosen,grains,workers_used,parcel_count,imbalance,probes,wins,gain_bytes,probe_cost,action_stats=encode(src,arc,tmp,model)
            ...
            erows.append(dict(epoch=epoch,corpus=cname,file=str(src.relative_to(croot)),raw=raw,size=size,ratio=size/raw,
                              ...
                              model_buckets=len(model),action_stats=action_stats,sha_ok=ok))
            dec.unlink()
...
Path("exp71_model.json").write_text(json.dumps(model,indent=2,sort_keys=True))
Path("exp71_results.json").write_text(json.dumps({"epochs":epochs,"model":model},indent=2))
