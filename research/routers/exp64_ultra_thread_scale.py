...
def encode(src,dst,tmp):
    ...
    workers=max(1,min(16,int(os.environ.get("KEPHIR_WORKERS","16"))))
...
root=Path("silesia");out=Path("exp64_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k64";dec=out/f"{fn}.dec"
...
Path("exp64_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
