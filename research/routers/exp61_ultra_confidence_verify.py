...
import subprocess,struct,hashlib,time,json,shutil,collections,math
...
def choose_mode(buf):
    # Return the selected mode plus a cheap confidence view of the sampled
    # structural candidates. EXP-61 uses this only to decide whether a
    # tightly-budgeted second structural encode is worth spending.
    ...
    ranked=sorted(cand,key=lambda x:x[0])
    h,mode=ranked[0]
...
def choose_grain(parent):
    ...
    if len(parent)>256*1024 and spread>=0.55:
        return 128*1024
    if len(parent)>256*1024 and spread>=0.25:
        return 256*1024
    return len(parent)

def encode(src,dst,tmp):
    raw=src.read_bytes();entries=[];chosen=collections.Counter();grains=collections.Counter();extra_checks=0
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            mode,ranked=choose_mode(chunk)
            payload=transform(chunk,mode)
            comp=cp(payload,tmp/f"p{pidx}_{j}_m{mode}")
            # Preserve EXP-58's BASE verification for structural transforms.
            if mode!=0:
            ...
            # EXP-61 single conceptual change: budgeted low-confidence
            # structural verification. On full 512 KiB chunks only, if the
            # two best sampled candidates are nearly tied, try exactly one
            # alternate structural mode. Hard cap: 32 extra backend encodes.
            if len(chunk)==CH and extra_checks<32:
                alts=[(h,m) for h,m in ranked if m!=mode]
                if alts:
                    best_h=ranked[0][0]
                    alt_h,alt_mode=alts[0]
                    if alt_h-best_h<=0.025 and alt_mode!=0:
                        alt_comp=cp(transform(chunk,alt_mode),tmp/f"p{pidx}_{j}_uncertain_m{alt_mode}")
                        extra_checks+=1
                        if len(alt_comp)<len(comp):
                            mode=alt_mode
                            comp=alt_comp

            # EXP-59 single conceptual change: on strongly text-like chunks,
            # try one reversible dictionary-token transform. Only spend the
            # extra backend encode when tokenization shrinks the raw payload
            # by at least 1%, then keep it only if the final archive is smaller.
            if is_text_like(chunk):
                ...
                if len(tok)+16 < len(chunk)*0.99:
                    ...
                    if len(tok_comp)<len(comp):
                        mode=6
                        comp=tok_comp
            chosen[mode]+=1;grains[len(chunk)]+=1
    ...
    return chosen,grains,extra_checks
...
root=Path("silesia");out=Path("exp61_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k61";dec=out/f"{fn}.dec"
    t=time.perf_counter();chosen,grains,extra_checks=encode(src,arc,tmp);ct=time.perf_counter()-t
...
modes=collections.Counter();grains=collections.Counter();extra_checks_total=0
for r in rows:
    modes.update(r["modes"]);grains.update(r["grains"]);extra_checks_total+=r["extra_checks"]
summary=dict(
    raw=raw,size=size,ratio=size/raw,
    comp_time_s=ct,dec_time_s=dt,
    comp_MBps=(raw/1e6)/ct,dec_MBps=(raw/1e6)/dt,
    sha_all=all(r["sha_ok"] for r in rows),
    modes=dict(modes),grains=dict(grains),extra_checks=extra_checks_total,
...
Path("exp61_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
