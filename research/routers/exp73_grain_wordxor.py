...
def encode(src,dst,tmp,model):
    ...
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            wx_probe=(not is_text_like(chunk)) and wx_should_probe(model,wx_key)
...
out=Path("exp73_out");tmp=out/"tmp";out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
model={}
epochs=[]
for epoch in range(1,4):
    ...
    for cname,croot in CORPORA:
        ...
        for fi,src in enumerate(files):
            safe=f"{cname}_{fi}"
            arc=out/f"e{epoch}_{safe}.k73";dec=out/f"e{epoch}_{safe}.dec"
            t=time.perf_counter()
...
Path("exp73_model.json").write_text(json.dumps(model,indent=2,sort_keys=True))
Path("exp73_results.json").write_text(json.dumps({"epochs":epochs,"model":model},indent=2))
