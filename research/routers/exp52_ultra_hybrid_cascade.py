...
# Canonical EXP-48 stages.
def stage(buf,name):
    if name=="BASE": return buf
    if name=="D2T2": return transpose(delta_lag(buf,2),2)
    if name=="D4T4": return transpose(delta_lag(buf,4),4)
    if name=="D16T16": return transpose(delta_lag(buf,16),16)
    if name=="D1024T1024": return transpose(delta_lag(buf,1024),1024)
    if name=="XOR2T2": return transpose(word_xor(buf,2),2)
    raise ValueError(name)

def inv_stage(buf,name,rawlen):
    if name=="BASE": return buf
    if name=="D2T2": return inv_delta(inv_transpose(buf,2,rawlen),2)
    if name=="D4T4": return inv_delta(inv_transpose(buf,4,rawlen),4)
    if name=="D16T16": return inv_delta(inv_transpose(buf,16,rawlen),16)
    if name=="D1024T1024": return inv_delta(inv_transpose(buf,1024,rawlen),1024)
    if name=="XOR2T2": return inv_word_xor(inv_transpose(buf,2,rawlen),2)
    raise ValueError(name)

# Single-stage EXP-48 candidates plus a small targeted set of two-stage cascades.
# Each tuple is applied left->right and inverted right->left.
CANDIDATES=[
    ("BASE", ("BASE",)),
    ("D2T2", ("D2T2",)),
    ("D4T4", ("D4T4",)),
    ("D16T16", ("D16T16",)),
    ("D1024T1024", ("D1024T1024",)),
    ("XOR2T2", ("XOR2T2",)),
    ("D2T2_D16T16", ("D2T2","D16T16")),
    ("D4T4_D16T16", ("D4T4","D16T16")),
    ("D16T16_D2T2", ("D16T16","D2T2")),
    ("D1024T1024_D2T2", ("D1024T1024","D2T2")),
    ("XOR2T2_D16T16", ("XOR2T2","D16T16")),
]

MODE_TO_DEF={i:c for i,c in enumerate(CANDIDATES)}

def transform(buf,mode):
    name,stages=MODE_TO_DEF[mode]
    out=buf
    for s in stages:
        out=stage(out,s)
    return out
...
root=Path("silesia"); out=Path("exp52_out"); tmp=out/"tmp"
out.mkdir(exist_ok=True); tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn; arc=out/f"{fn}.k52"; dec=out/f"{fn}.dec"
...
Path("exp52_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
