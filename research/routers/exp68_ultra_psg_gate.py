...
def process_parcel(args):
    ...
    for seq,pidx,j,chunk in parcel:
        ...
        # EXP-67: spend the recovered runtime budget only on non-text chunks.
        # Verify the final selected transform with the two complementary PSG
        # backends retained from EXP-53/55 and keep the smallest archive.
        if (not text_like) and mode==0:
            payload=chunk
            c53=cp(payload,tmp/f"p{pidx}_{j}_psg53","./kephir53")
            c55=cp(payload,tmp/f"p{pidx}_{j}_psg55","./kephir55")
            if len(c53)<len(comp):
                backend=1;comp=c53
            if len(c55)<len(comp):
                backend=2;comp=c55
        ...
        out.append((seq,backend,mode,len(chunk),comp))
    return out
...
def encode(src,dst,tmp):
    ...
    entries=[(backend,mode,n,comp) for _,backend,mode,n,comp in flat]
...
root=Path("silesia");out=Path("exp68_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k68";dec=out/f"{fn}.dec"
    ...
    row=dict(file=fn,raw=raw,size=size,ratio=size/raw,
             ...
             modes={f"{k[0]}:{k[1]}":v for k,v in sorted(chosen.items())},
...
summary=dict(
    ...
    baseline_exp67_size=63454523,
    delta_vs_exp67_bytes=size-63454523,
    ratio_delta_vs_exp67_pp=(size/raw-63454523/raw)*100.0,
...
Path("exp68_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
