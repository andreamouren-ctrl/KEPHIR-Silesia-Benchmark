...
def encode(src,dst,tmp,model):
    ...
    return chosen,grains,workers,len(parcels),imbalance,grain_probes,grain_wins,grain_gain_bytes,grain_probe_cost_s,dict(grain_actions)
...
out=Path("exp72_out");tmp=out/"tmp";out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
model={}
epochs=[]
for epoch in range(1,4):
    ...
    for cname,croot in CORPORA:
        ...
        for fi,src in enumerate(files):
            safe=f"{cname}_{fi}"
            arc=out/f"e{epoch}_{safe}.k72";dec=out/f"e{epoch}_{safe}.dec"
            t=time.perf_counter()
...
Path("exp72_model.json").write_text(json.dumps(model,indent=2,sort_keys=True))
Path("exp72_results.json").write_text(json.dumps({"epochs":epochs,"model":model},indent=2))
