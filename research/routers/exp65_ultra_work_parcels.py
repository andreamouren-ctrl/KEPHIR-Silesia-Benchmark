        ...
        if is_text_like(chunk):
            ...
            if len(tok)+16 < len(chunk)*0.99:
                ...
                if len(tok_comp)<len(comp):
                    mode=6
                    comp=tok_comp
        out.append((mode,len(chunk),comp))
    return out
...
def encode(src,dst,tmp):
    ...
    # EXP-65: split the task list into a modest number of coarse work parcels.
    # Each process handles several chunks sequentially, reducing scheduler/IPC
    # overhead while removing the Python GIL from tokenization and transforms.
    target_parcels=max(workers,min(len(tasks),workers*3))
    parcel_size=max(1,math.ceil(len(tasks)/target_parcels))
    parcels=[tasks[i:i+parcel_size] for i in range(0,len(tasks),parcel_size)]

    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as ex:
        parcel_results=list(ex.map(process_parcel,[(p,str(tmp)) for p in parcels],chunksize=1))
    entries=[entry for parcel in parcel_results for entry in parcel]
    ...
    return chosen,grains,workers,len(parcels),parcel_size
...
root=Path("silesia");out=Path("exp65_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k65";dec=out/f"{fn}.dec"
    t=time.perf_counter();chosen,grains,workers_used,parcel_count,parcel_size=encode(src,arc,tmp);ct=time.perf_counter()-t
    ...
    row=dict(file=fn,raw=raw,size=size,ratio=size/raw,
             ...
             workers_used=workers_used,parcel_count=parcel_count,parcel_size=parcel_size,
             sha_ok=ok)
...
Path("exp65_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
