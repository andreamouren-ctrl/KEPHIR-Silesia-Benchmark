...
def encode(src,dst,tmp):
    ...
    for idx,start in enumerate(range(0,len(raw),CH)):
        ...
        for mode in range(6):
            ...
            for backend,exe in ((0,"./kephir37"),(1,"./kephir53")):
...
def decode(src,dst,tmp):
    ...
    for idx in range(count):
        ...
        exe="./kephir53" if backend else "./kephir37"
...
root=Path("silesia");out=Path("exp54_out");tmp=out/"tmp"
out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
rows=[]
for fn in FILES:
    src=root/fn;arc=out/f"{fn}.k54";dec=out/f"{fn}.dec"
    ...
    for (backend,mode),count in sorted(chosen.items()):
        labels[f"{'EXP53' if backend else 'EXP48'}_M{mode}"]=count
...
Path("exp54_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
