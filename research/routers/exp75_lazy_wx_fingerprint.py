...
def chunk_feature_bucket(buf,coarse):
    # EXP-75: this expensive fine fingerprint is called lazily only after
    # the cheap factory-prior coarse gate has accepted the chunk.
...
def encode(src,dst,tmp,model):
    ...
    for pidx,start in enumerate(range(0,len(raw),CH)):
        ...
        for j,off in enumerate(range(0,len(parent),grain)):
            ...
            wx_key=wx_coarse
            wx_probe=False
            if wx_coarse in FACTORY_WX_COARSE and not is_text_like(chunk):
                wx_key=chunk_feature_bucket(chunk,wx_coarse)
                wx_probe=wx_should_probe(model,wx_key)
...
out=Path("exp75_out");tmp=out/"tmp";out.mkdir(exist_ok=True);tmp.mkdir(exist_ok=True)
model={}
epochs=[]
for epoch in range(1,4):
    ...
    for cname,croot in CORPORA:
        ...
        for fi,src in enumerate(files):
            safe=f"{cname}_{fi}"
            arc=out/f"e{epoch}_{safe}.k75";dec=out/f"e{epoch}_{safe}.dec"
            t=time.perf_counter()
...
Path("exp75_model.json").write_text(json.dumps(model,indent=2,sort_keys=True))
Path("exp75_results.json").write_text(json.dumps({"epochs":epochs,"model":model},indent=2))
