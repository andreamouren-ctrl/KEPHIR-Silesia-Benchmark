...
from kstream_video_motion_field_v2 import encode_file, decode_file, RAW8, INDEX5, XY3_SPLIT

OUT=Path("results/video/ksv06_motion_field")
...
def bench_one(src,name,fpsn,fpsd,mode,label,exe):
    front=OUT/f"{name}_{label}.front"
    dec=OUT/f"{name}_{label}.dec.yuv"
    arc=OUT/f"{name}_{label}.aur"
    outdir=OUT/f"{name}_{label}_out"
    t=time.perf_counter()
    encode_file(src,front,W,H,fpsn,fpsd,GOP,BLOCK,RADIUS,mode)
    fe=time.perf_counter()-t
    t=time.perf_counter(); decode_file(front,dec); fd=time.perf_counter()-t
    ...
    frames=src.stat().st_size//FS
    dur=frames*fpsd/fpsn
...
def main():
    ...
    for spec in a.clip:
        ...
        rows=[
            bench_one(src,name,fpsn,fpsd,RAW8,"RAW8",a.kephir),
            bench_one(src,name,fpsn,fpsd,INDEX5,"INDEX5",a.kephir),
            bench_one(src,name,fpsn,fpsd,XY3_SPLIT,"XY3_SPLIT",a.kephir),
        ]
        base=rows[0]["final_bytes"]
        for r in rows:
            r["delta_vs_raw8_bytes"]=r["final_bytes"]-base
            r["delta_vs_raw8_percent"]=100*(r["final_bytes"]-base)/base
        clips.append(dict(name=name,raw_bytes=src.stat().st_size,rows=rows))
        print(clips[-1],flush=True)
    result={"experiment":"KSV-06 motion-field representation","clips":clips,
            "decision_rule":"Promote only if final KHEPRI bytes improve and roundtrip remains bit-exact."}
    (OUT/"KSV06_RESULTS.json").write_text(json.dumps(result,indent=2))
