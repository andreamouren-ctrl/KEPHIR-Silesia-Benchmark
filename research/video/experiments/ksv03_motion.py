...
from kstream_video_baseline import encode_file as temporal_encode, decode_file as temporal_decode
from kstream_video_mc import encode_file as mc_encode, decode_file as mc_decode

OUT=Path("streaming/ksv03_out")
W=176; H=144; GOP=10
...
def temporal(exe,raw,name,fpsn,fpsd):
    ...
    t=time.perf_counter(); temporal_encode(raw,front,W,H,fpsn,fpsd,GOP,3); fe=time.perf_counter()-t
    t=time.perf_counter(); temporal_decode(front,restored); fd=time.perf_counter()-t
    if sha(restored)!=sha(raw): raise SystemExit(f"TEMPORAL SHA FAIL {name}")
    kb,ke,kd=kephir(exe,front,f"{name}_TEMPORAL")
    return dict(codec="TEMPORAL_GOP10",bytes=kb,frontend_bytes=front.stat().st_size,
                encode_seconds=fe+ke,decode_seconds=fd+kd,sha_ok=True)

def motion(exe,raw,name,fpsn,fpsd,block):
    tag=f"{name}_MC{block}_R4"
    front=OUT/f"{tag}.front"; restored=OUT/f"{tag}.raw"
    t=time.perf_counter(); mc_encode(raw,front,W,H,fpsn,fpsd,GOP,block,4); fe=time.perf_counter()-t
    t=time.perf_counter(); mc_decode(front,restored); fd=time.perf_counter()-t
    if sha(restored)!=sha(raw): raise SystemExit(f"MC SHA FAIL {tag}")
    kb,ke,kd=kephir(exe,front,tag)
    return dict(codec=f"MC{block}_R4_GOP10",bytes=kb,frontend_bytes=front.stat().st_size,
                encode_seconds=fe+ke,decode_seconds=fd+kd,sha_ok=True)
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--kephir",default="./kephir33")
    ...
    for spec in a.clip:
        ...
        rows=[
            ...
            motion(a.kephir,raw,name,fpsn,fpsd,16),
            motion(a.kephir,raw,name,fpsn,fpsd,8),
    ...
    result=dict(experiment="KS-V03 motion-compensation controls",gop=GOP,
                clips=clips,rule="Motion compensation is BACKGROUND. This experiment only establishes a technical baseline.")
    (OUT/"ksv03_results.json").write_text(json.dumps(result,indent=2))
