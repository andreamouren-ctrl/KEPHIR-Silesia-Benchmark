#!/usr/bin/env python3
"""
KSV-24 — H3 residual-mapping selector.
...
KSV-24 therefore separates the decisions:
...
OUT=Path("results/video/ksv24_residual_mapping_selector")
THRESHOLD=2.60
...
def build_window(chunk,exe,tmp,w,h,fpsn,fpsd,gop,tag):
    ...
    mapping=(
        k17.DENSE_ZZ if mean_mag<THRESHOLD
        else k17.DENSE_MOD8
    )
...
def encode_source(src,prefix,exe,w,h,fpsn,fpsd,gop=10,route_span=20):
    ...
    with tempfile.TemporaryDirectory(prefix="ksv24_") as td:
        tmp=Path(td)
...
def main():
    ...
    result={
        "experiment":"KSV-24 H3 residual mapping selector",
        "mapping_threshold":THRESHOLD,
        "mapping_rule":"ZZ_INTER if mean_signed_residual_magnitude < threshold else MOD8",
        "chroma_rule":"first-order entropy over FLOOR/TRUNC MOD8 H3 frontend",
        "production_candidate_duplicate_khepri_encodes":0,
        "rows":rows,
        "aggregate":aggregate,
    }
    (OUT/"KSV24_RESULTS.json").write_text(json.dumps(result,indent=2))

    print("KSV24_RESIDUAL_MAPPING_SELECTOR_PASS")
    print(
        "KSV24_AGGREGATE",
        "baseline",baseline,
        "selected",selected,
