#!/usr/bin/env python3
"""
KSV-19 H2 — Hierarchical Dense Integer Motion.
...
so KSV-19 H2 reuses the K17D wire format and decoder unchanged.
...
OUT = Path("results/video/ksv19_hierarchical_dense_h2")
...
MODE_R4_MOD8 = 1
MODE_R4_ZZ = 2
MODE_H2_FLOOR_MOD8 = 3
MODE_H2_FLOOR_ZZ = 4
MODE_H2_TRUNC_MOD8 = 5
MODE_H2_TRUNC_ZZ = 6
...
def _best_h2_luma(
    ...
    # Stage 1: evaluate the existing 25 sparse-even vectors exactly.
    for dx, dy in SPARSE:
        ...
        # Zero SAD is globally optimal regardless of any refinement.
        if cost == 0:
            return cost, dense_index, dx, dy, len(evaluated)

    if not sparse_ranked:
        raise RuntimeError("H2 sparse stage found no valid candidate")

    sparse_ranked.sort(key=lambda item: item[0])
    seeds = sparse_ranked[:2]
    best = seeds[0]

    # Stage 2: union the unique 3x3 integer neighborhoods around the
    # two best sparse-even vectors. The final tie order is still the
    # canonical dense KSV-17 index order.
    neighborhood = set()
    for _, seed_dx, seed_dy in seeds:
        for dy in range(
            max(-RADIUS, seed_dy - 1),
            min(RADIUS, seed_dy + 1) + 1,
        ):
            for dx in range(
                max(-RADIUS, seed_dx - 1),
                min(RADIUS, seed_dx + 1) + 1,
            ):
                neighborhood.add((dx, dy))

    for dx, dy in sorted(neighborhood, key=lambda v: DENSE_INDEX[v]):
...
def h2_motion_residuals(
    ...
    for by in range(bh):
        ...
        for bx in range(bw):
            ...
            cost, index, dx, dy, evals = _best_h2_luma(
                cy, py, x0, y0, w, h
            )
...
def build_h2_records(raw_chunk: bytes, w: int, h: int, gop: int):
    ...
    while off < total:
        ...
        for j in range(n):
            ...
            if previous is None:
                ...
                motion, residuals, st = h2_motion_residuals(
                    frame, previous, w, h
                )
                records.append((
                    "P",
...
def h2_candidates_for_window(
    ...
    chunks, stats, search_seconds = build_h2_records(
        raw_chunk, w, h, gop
    )
    ...
    rows = []
    configs = (
        (k17.CHROMA_FLOOR, k17.DENSE_MOD8, "H2_FLOOR_MOD8", MODE_H2_FLOOR_MOD8),
        (k17.CHROMA_FLOOR, k17.DENSE_ZZ, "H2_FLOOR_ZZ", MODE_H2_FLOOR_ZZ),
        (k17.CHROMA_TRUNC, k17.DENSE_MOD8, "H2_TRUNC_MOD8", MODE_H2_TRUNC_MOD8),
        (k17.CHROMA_TRUNC, k17.DENSE_ZZ, "H2_TRUNC_ZZ", MODE_H2_TRUNC_ZZ),
    )
...
def decode_outer(src: Path, dst: Path, exe: Path):
    ...
    if len(data) < OUTER_HDR.size:
        raise ValueError("truncated KSV-19 outer header")
    ...
    if magic != OUTER_MAGIC or version != OUTER_VERSION:
        raise ValueError("bad KSV-19 outer stream")
    ...
    with tempfile.TemporaryDirectory(prefix="ksv19_dec_") as td:
        ...
        while pos < len(data):
            if pos + OUTER_ENT.size > len(data):
                raise ValueError("truncated KSV-19 outer entry")

            mode, frames, payload_size = OUTER_ENT.unpack_from(data, pos)
            pos += OUTER_ENT.size

            if pos + payload_size > len(data):
                raise ValueError("truncated KSV-19 outer payload")
            ...
            if mode == MODE_TEMP:
                ...
                raise ValueError("unknown KSV-19 mode")

            if len(chunk) != frames * k13.frame_size(w, h):
                raise ValueError("KSV-19 decoded frame count mismatch")
...
def encode_source(
    ...
    with tempfile.TemporaryDirectory(prefix="ksv19_enc_") as td:
        ...
        for window_index, off in enumerate(range(0, total, route_span)):
            ...
            h2, h2_seconds = h2_candidates_for_window(
                chunk, exe, tmp, w, h, fpsn, fpsd, gop, f"w{window_index}"
            )
            ...
            h2_entries.append(dict(h2_policy))
            ...
            h2_cost = h2_seconds + sum(x["backend_seconds"] for x in h2)
    ...
    return {
        ...
        "h2": {
            "bytes": paths["h2"].stat().st_size,
            "encode_research_seconds": h2_research_seconds,
            "modes": policy_stats(h2_entries),
        },
        "dense": {
...
def main():
    ...
    for clip in args.clip:
        ...
        decoded = {}
        for policy in ("baseline", "h2", "dense"):
        ...
        h2_gain = row["baseline"]["bytes"] - row["h2"]["bytes"]

        row["h2_delta_percent"] = (
            100.0 * (row["h2"]["bytes"] / row["baseline"]["bytes"] - 1.0)
        )
        row["dense_delta_percent"] = (
            100.0 * (row["dense"]["bytes"] / row["baseline"]["bytes"] - 1.0)
        )
        row["gain_recovered_percent"] = (
            100.0 * h2_gain / dense_gain if dense_gain > 0 else 0.0
        )
        ...
        print(
            ...
            "h2_delta_pct", f"{row['h2_delta_percent']:.6f}",
    ...
    h2_gain = aggregate["baseline"]["bytes"] - aggregate["h2"]["bytes"]

    aggregate["h2_delta_percent"] = (
        100.0 * (
            aggregate["h2"]["bytes"] / aggregate["baseline"]["bytes"] - 1.0
        )
    )
    ...
    aggregate["h2_vs_dense_search_time_ratio"] = (
        aggregate["h2"]["encode_research_seconds"]
    ...
    result = {
        "experiment": "KSV-19 H2 hierarchical dense refinement",
        "sparse_candidate_count": 25,
        "dense_candidate_count": 81,
        "hierarchical_policy": "top-2 sparse-even vectors + union of unique 3x3 integer neighborhoods",
        "rows": rows,
        "aggregate": aggregate,
        "notes": [
            "H2 and exhaustive dense use the same K17D motion-map semantics.",
            "H2 evaluates approximately 25-41 candidates/block instead of 81.",
    ...
    print(
        ...
        "h2", aggregate["h2"]["bytes"],
        "dense", aggregate["dense"]["bytes"],
        "h2_delta_pct", f"{aggregate['h2_delta_percent']:.6f}",
        "dense_delta_pct", f"{aggregate['dense_delta_percent']:.6f}",
        "recovered_pct", f"{aggregate['gain_recovered_percent']:.3f}",
        "h2_vs_dense_research_time_ratio",
        f"{aggregate['h2_vs_dense_search_time_ratio']:.4f}",
    )
