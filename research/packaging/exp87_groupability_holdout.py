...
Purpose:
validate the frozen EXP-86 groupability thresholds on unseen Router Matrix v2
holdout workloads.

No threshold changes are permitted in this experiment.
No SMART/FLAT compression probe is used for routing.
...
OUT=ROOT/"exp87_groupability_holdout"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp87_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP87 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP87 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp87_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp87_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp87_smart_dec_") as td:
...
result={
    "experiment":"EXP-87",
    "change":"groupability-holdout-validation",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP87_BEGIN",label,flush=True)
...
Path("exp87_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP87_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP87_SHA_ALL_PASS",flush=True)
