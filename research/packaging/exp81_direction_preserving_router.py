...
Purpose:
fix the single EXP-80 routing error without regressing its seven correct
decisions.

Policy change:
content diversity may reinforce a near-tie only when the measured probe already
leans SMART. A near-tie that leans FLAT is never overridden to SMART; instead
it escalates to the bounded stage-2 probe.
...
OUT=ROOT/"exp81_direction_router"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp81_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP81 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP81 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp81_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp81_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp81_smart_dec_") as td:
...
def route_exp79(root,label):
    total=logical_bytes(root)
    stage1=probe_layout(root,label+".s1",min(STAGE1_BUDGET,total))

    if stage1["sampled_bytes"]>=total:
        return stage1["selected_layout"],"stage1-full-input",[stage1]

    if stage1["relative_margin"]>=STAGE1_CONFIDENCE:
        return stage1["selected_layout"],"stage1-strong-margin",[stage1]

    if (
        stage1["selected_layout"]=="smart"
        and diversity_prefers_smart(stage1["diversity"],total)
    ):
        return "smart","stage1-ambiguous-smart-confirmed-by-diversity",[stage1]

    # Direction-preserving uncertainty policy:
    # a near-tie that currently leans FLAT is never flipped solely because the
    # sample is diverse. Escalate instead and let the larger bounded probe
    # resolve it.
    ...
    return selected,reason,[stage1,stage2]
...
result={
    "experiment":"EXP-81",
    "change":"direction-preserving-uncertainty-router",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP81_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]

    t0=time.perf_counter()
    selected,reason,stages=route_exp79(root,label)
...
Path("exp81_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP81_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP81_SHA_ALL_PASS",flush=True)
