...
Purpose:
reduce AUTO routing cost while preserving EXP-82's zero-regret behavior.

New fast path:
if the original directory resolves to exactly one KEPHIR content family,
SMART grouping collapses to one stream and cannot create a compression-context
advantage over FLAT. Select FLAT before any compression probe.

The repository workload is frozen to a pinned general-purpose source snapshot
so experiment-to-experiment comparisons are reproducible.
...
OUT=ROOT/"exp83_fast_auto_router"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp83_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP83 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP83 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp83_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp83_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp83_smart_dec_") as td:
...
def route_exp83(root,label):
    ...
    stage1=probe_layout(root,label+".s1",min(STAGE1_BUDGET,total))
...
result={
    "experiment":"EXP-83",
    "change":"single-group-fast-path-and-frozen-matrix",
...
for label,root in datasets.items():
    print("EXP83_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]

    t0=time.perf_counter()
    selected,reason,stages,root_diversity=route_exp83(root,label)
...
Path("exp83_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP83_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP83_SHA_ALL_PASS",flush=True)
