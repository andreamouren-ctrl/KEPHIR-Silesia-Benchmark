...
Purpose:
combine EXP-86's cheap structural gate with EXP-84's bounded compression probe.

The EXP-86 thresholds remain frozen:
- if the cheap estimator says FLAT, accept FLAT directly;
- if the cheap estimator says SMART, require EXP-84 bounded measured evidence.

No threshold tuning from holdout labels is permitted.
...
OUT=ROOT/"exp88_hybrid_groupability_gate"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp88_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP88 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP88 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp88_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp88_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp88_smart_dec_") as td:
...
def route_exp88(root,label):
    ...
    if not smart_candidate:
        return "flat","groupability-flat-fast-path",[],features
    ...
    if stage1["sampled_bytes"]>=total:
        return stage1["selected_layout"],"groupability-smart-full-input-probe",[stage1],features

    if stage1["relative_margin"]>=STAGE1_CONFIDENCE:
        return stage1["selected_layout"],"groupability-smart-strong-probe",[stage1],features

    if diversity_prefers_smart(stage1["diversity"],total):
        return stage1["selected_layout"],"groupability-smart-representative-probe",[stage1],features

    stage2=probe_layout(root,label+".s2",min(STAGE2_BUDGET,total))
    return stage2["selected_layout"],"groupability-smart-stage2-probe",[stage1,stage2],features
...
result={
    "experiment":"EXP-88",
    "change":"hybrid-groupability-gate",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP88_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]

    t0=time.perf_counter()
    selected,reason,stages,groupability=route_exp88(root,label)
...
base_names=set(datasets)-set(RM.HOLDOUT_NAMES)
holdout_names=set(RM.HOLDOUT_NAMES)

def aggregate(names):
    rows=[result["datasets"][name] for name in names]
    return {
        "datasets":len(rows),
        "correct_selections":sum(r["selected_layout"]==r["oracle_layout"] for r in rows),
        "selection_accuracy":(
            sum(r["selected_layout"]==r["oracle_layout"] for r in rows)/len(rows)
            if rows else 0.0
        ),
        "total_regret_bytes":sum(r["selection_regret_bytes"] for r in rows),
        "total_routing_time_s":sum(r["routing_time_s"] for r in rows),
    }

result["aggregate"]={
    "all":aggregate(datasets.keys()),
    "matrix_v1":aggregate(base_names),
    "holdout_v2":aggregate(holdout_names),
}

Path("exp88_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP88_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP88_SHA_ALL_PASS",flush=True)
