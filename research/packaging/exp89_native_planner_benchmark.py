...
def main():
    if len(sys.argv)!=2:
        raise SystemExit("usage: exp89_native_planner_benchmark.py /path/to/kephir2_planner_dump")
    ...
    out={
        "experiment":"EXP-89",
        "change":"native-planner-gate-parity-performance",
        "aggregate":aggregate,
        "datasets":results,
    }
    Path("exp89_results.json").write_text(json.dumps(out,indent=2,sort_keys=True))

    print("EXP89_AGGREGATE",json.dumps(aggregate,sort_keys=True),flush=True)
    ...
    print("EXP89_NATIVE_PARITY_PASS",flush=True)
