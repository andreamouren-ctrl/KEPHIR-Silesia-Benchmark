...
Purpose:
combine EXP-82 correctness with EXP-83 reduced probe budgets.

New production-oriented fast paths:
- a directory whose files all classify into one content family selects FLAT
  without a compression probe because SMART would feed the same concatenated
  payload to the same inner engine while carrying extra group metadata;
- directories <= 1 MiB use a full-input probe instead of a partial sample;
- larger heterogeneous directories retain 512 KiB / 2 MiB bounded probing.

The workload matrix is frozen through router_matrix_v1.
...
OUT=ROOT/"exp84_deterministic_dominance_router"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp84_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP84 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP84 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp84_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp84_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp84_smart_dec_") as td:
...
def single_content_class(root):
    first=None
    count=0
    for p in files(root):
        count+=1
        group=K.classify(p.read_bytes())
        if first is None:
            first=group
        elif group!=first:
            return False,count
...
def route_exp84(root,label):
    total=logical_bytes(root)

    # Deterministic dominance rule: with one content class SMART creates one
    # group containing the same file concatenation as FLAT, then adds group
    # metadata. No compression probe is needed.
    one_class,classified_files=single_content_class(root)
    if one_class:
        return "flat","single-content-class-dominance",[],classified_files

    stage1_budget=total if total<=SMALL_FULL_PROBE_LIMIT else min(STAGE1_BUDGET,total)
    stage1=probe_layout(root,label+".s1",stage1_budget)

    if stage1["sampled_bytes"]>=total:
        return stage1["selected_layout"],"stage1-full-input",[stage1],classified_files

    if stage1["relative_margin"]>=STAGE1_CONFIDENCE:
        return stage1["selected_layout"],"stage1-strong-margin",[stage1],classified_files

    if diversity_prefers_smart(stage1["diversity"],total):
        return stage1["selected_layout"],"stage1-representative-content-diversity",[stage1],classified_files

    stage2=probe_layout(root,label+".s2",min(STAGE2_BUDGET,total))
    return stage2["selected_layout"],"stage2-measured-direction",[stage1,stage2],classified_files
...
result={
    "experiment":"EXP-84",
    "change":"deterministic-dominance-fast-router",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP84_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]

    t0=time.perf_counter()
    selected,reason,stages,classified_files=route_exp84(root,label)
    ...
    result["datasets"][label]={
        ...
        "probe_stages":stages,
        "preclassified_files":classified_files,
        "smart":smart,
        "flat":flat,
...
Path("exp84_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP84_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP84_SHA_ALL_PASS",flush=True)
