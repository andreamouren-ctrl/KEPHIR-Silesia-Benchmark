#!/usr/bin/env python3
"""
EXP-85 — Adaptive-Budget Fast AUTO Router

Purpose:
combine the validated single-content-group fast path with adaptive probe budgets.

Policy:
- one content family -> FLAT without compression probing;
- multi-class archive <= 2 MiB -> full-input probe;
- larger multi-class archive -> 512 KiB stage 1;
- ambiguous evidence -> bounded 2 MiB stage 2.

The canonical general-purpose repository workload remains pinned.
...
OUT=ROOT/"exp85_adaptive_budget_router"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
SMALL_FULL_PROBE_THRESHOLD=2*1024*1024
MIN_FILE_SAMPLE=4096
STRATA=4

PINNED_REPO_COMMIT="31f7e6099cec307ad934fd9ec8e6760441567ab5"
CANONICAL_REPO_PREFIXES=(
    "engine/",
    "release/",
...
def tracked_snapshot():
    """Frozen general-purpose repository workload from a pinned commit."""
    dst=INPUTS/"repository"
    dst.mkdir()

    raw=subprocess.check_output(
        ["git","ls-tree","-r","-z","--name-only",PINNED_REPO_COMMIT]
    )
    for item in raw.split(b"\0"):
        if not item:
            continue
        rel=item.decode("utf-8")
        if not rel.startswith(CANONICAL_REPO_PREFIXES):
            continue
        try:
            data=subprocess.check_output(
                ["git","show",f"{PINNED_REPO_COMMIT}:{rel}"]
            )
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp85_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP85 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP85 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp85_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp85_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp85_smart_dec_") as td:
...
def route_exp85(root,label):
    total=logical_bytes(root)

    # Structural fast path: if all original files classify into one content
    # family, SMART has only one payload group. It cannot improve compression
    # context versus FLAT, while it carries additional grouping metadata.
    diversity=content_diversity(root)
    if diversity["group_count"]<=1:
        return "flat","single-content-group-fast-path",[],diversity

    stage1_budget = total if total<=SMALL_FULL_PROBE_THRESHOLD else min(STAGE1_BUDGET,total)
    stage1=probe_layout(root,label+".s1",stage1_budget)

    if stage1["sampled_bytes"]>=total:
        return stage1["selected_layout"],"stage1-full-input",[stage1],diversity

    if stage1["relative_margin"]>=STAGE1_CONFIDENCE:
        return stage1["selected_layout"],"stage1-strong-margin",[stage1],diversity

    if diversity_prefers_smart(stage1["diversity"],total):
        return stage1["selected_layout"],"stage1-representative-content-diversity",[stage1],diversity

    stage2=probe_layout(root,label+".s2",min(STAGE2_BUDGET,total))
    return stage2["selected_layout"],"stage2-measured-direction",[stage1,stage2],diversity
...
datasets={
    "repository":tracked_snapshot(),
    "silesia":ROOT/"corpora"/"silesia",
    "many_tiny_source":make_many_tiny_source(),
    "homogeneous_large":make_homogeneous_large(),
    "mixed_content":make_mixed_content(),
    "incompressible":make_incompressible(),
    "zero_rich":make_zero_rich(),
    "redundant_backup":make_redundant_backup(),
}
...
result={
    "experiment":"EXP-85",
    "change":"single-group-fast-path-plus-adaptive-probe-budget",
    "pinned_repository_commit":PINNED_REPO_COMMIT,
    "canonical_repository_prefixes":list(CANONICAL_REPO_PREFIXES),
    "thresholds":{
        "stage1_budget":STAGE1_BUDGET,
        "small_full_probe_threshold":SMALL_FULL_PROBE_THRESHOLD,
...
for label,root in datasets.items():
    print("EXP85_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]

    t0=time.perf_counter()
    selected,reason,stages,root_diversity=route_exp85(root,label)
...
Path("exp85_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP85_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP85_SHA_ALL_PASS",flush=True)
