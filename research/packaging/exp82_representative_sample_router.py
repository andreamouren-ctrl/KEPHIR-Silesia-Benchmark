...
Purpose:
treat content diversity as a confidence signal rather than a SMART preference.

A sufficiently diverse bounded sample is considered representative, so its
measured SMART/FLAT direction is preserved even when the margin is small.
Homogeneous/uncertain samples escalate to the larger bounded stage-2 probe.
Stage 2 always preserves the measured direction.
...
OUT=ROOT/"exp82_representative_router"
if OUT.exists():
    shutil.rmtree(OUT)
OUT.mkdir()
INPUTS=OUT/"inputs"
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp82_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP82 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP82 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp82_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp82_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp82_smart_dec_") as td:
...
def route_exp79(root,label):
    ...
    if diversity_prefers_smart(stage1["diversity"],total):
        return stage1["selected_layout"],"stage1-representative-content-diversity",[stage1]

    stage2=probe_layout(root,label+".s2",min(STAGE2_BUDGET,total))
    # Stage 2 is the maximum bounded evidence. Preserve what was measured;
    # diversity is not allowed to choose a different layout.
    selected=stage2["selected_layout"]
    reason="stage2-measured-direction"
...
result={
    "experiment":"EXP-82",
    "change":"representative-sample-direction-router",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP82_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]
...
Path("exp82_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP82_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP82_SHA_ALL_PASS",flush=True)
