...
Purpose:
stress the EXP-79 uncertainty-aware SMART/FLAT routing principle on diverse
real and deterministic synthetic general-purpose directory workloads.

The routing decision is content-first. Synthetic files intentionally use the
same neutral .dat extension so extension/name cannot carry the decision.
...
def compress_flat(root,out):
    ...
    with tempfile.TemporaryDirectory(prefix="exp80_flat_") as td:
        td=Path(td)
        raw=td/"flat.raw"
        inner=td/"flat.k75"
...
def extract_flat(arc,out):
    b=arc.read_bytes()
    if b[:4]!=FLAT_MAGIC:
        raise ValueError("not EXP80 flat")
    pos=4
    version=b[pos]
    pos+=1
    if version!=FLAT_VERSION:
        raise ValueError("unsupported EXP80 flat version")
    ...
    with tempfile.TemporaryDirectory(prefix="exp80_flat_dec_") as td:
        td=Path(td)
        inner=td/"flat.k75"
        raw=td/"flat.raw"
...
def run_smart(root,label):
    ...
    with tempfile.TemporaryDirectory(prefix="exp80_smart_") as td:
        stats=K.compress_directory(root,arc,fresh_factory_model(),Path(td))
    comp=time.perf_counter()-t0
    t0=time.perf_counter()
    with tempfile.TemporaryDirectory(prefix="exp80_smart_dec_") as td:
...
def route_exp79(root,label):
    ...
    if diversity_prefers_smart(stage1["diversity"],total):
        return "smart","stage1-ambiguous-content-diversity",[stage1]
...
result={
    "experiment":"EXP-80",
    "change":"global-router-validation-matrix",
    "thresholds":{
...
for label,root in datasets.items():
    print("EXP80_BEGIN",label,flush=True)
    total=logical_bytes(root)
    fs=files(root)
    sizes=[p.stat().st_size for p in fs]
...
Path("exp80_results.json").write_text(json.dumps(result,indent=2,sort_keys=True))
print("EXP80_AGGREGATE",json.dumps(result["aggregate"],sort_keys=True),flush=True)
print("EXP80_SHA_ALL_PASS",flush=True)
