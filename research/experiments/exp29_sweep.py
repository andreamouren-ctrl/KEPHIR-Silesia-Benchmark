...
ROOT=Path("silesia"); OUT=Path("exp29_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP27A",0),("EXP29A_FULL_COMPETE",1),("EXP29B_55P",2),("EXP29C_35P",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir29_{mode}"
...
Path("exp29_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
