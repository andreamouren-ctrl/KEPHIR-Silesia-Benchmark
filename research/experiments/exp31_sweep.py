...
ROOT=Path("silesia"); OUT=Path("exp31_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP30A_FIXED50",4),("EXP31A_ASYM",1),("EXP31B_SMOOTH",2),("EXP31C_3ZONE",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir31_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp31_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
