...
ROOT=Path("silesia"); OUT=Path("exp38_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP37A_TWO_PASS",0),("EXP38A_FUSED_CONS",1),("EXP38B_FUSED_BAL",2),("EXP38C_FUSED_MID",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir38_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp38_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
