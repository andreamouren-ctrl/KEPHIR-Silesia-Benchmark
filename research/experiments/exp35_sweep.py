...
ROOT=Path("silesia"); OUT=Path("exp35_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP33H_BASE",0),("EXP35A_D255",1),("EXP35B_D4095",2),("EXP35C_D65535",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir35_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp35_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
