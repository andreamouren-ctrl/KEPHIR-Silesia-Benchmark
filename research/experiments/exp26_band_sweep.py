...
ROOT=Path("silesia"); OUT=Path("exp26_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP24B",2),("EXP26A",7),("EXP26B",8),("EXP26C",9)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir26_{mode}"
...
Path("exp26_band_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
