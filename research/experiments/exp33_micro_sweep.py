...
ROOT=Path("silesia"); OUT=Path("exp33_micro_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP33B_BASE",2),("EXP33D_LIGHT",4),("EXP33E_STRONG",5),("EXP33F_PLANE",6)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir33m_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp33_micro_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
