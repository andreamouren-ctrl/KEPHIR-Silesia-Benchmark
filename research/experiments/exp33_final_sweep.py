...
ROOT=Path("silesia"); OUT=Path("exp33_final_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP33E_STRONG",5),("EXP33G_STRONGER",7),("EXP33H_AGGR",8),("EXP33I_PLANE_STRONG",9)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir33f_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp33_final_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
