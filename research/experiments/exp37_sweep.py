...
ROOT=Path("silesia"); OUT=Path("exp37_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP33H_BASE",0),("EXP37A_CONSERVATIVE",1),("EXP37B_BALANCED",2),("EXP37C_AGGRESSIVE",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir37_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp37_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
