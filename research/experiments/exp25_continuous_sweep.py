...
ROOT=Path("silesia"); OUT=Path("exp25_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP24B",2),("EXP25A_TEXT_CONT",4),("EXP25B_TEXT_ENT",5),("EXP25C_HYBRID_CONT",6)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir25_{mode}"
...
Path("exp25_continuous_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
