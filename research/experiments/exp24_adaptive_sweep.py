...
ROOT=Path("silesia"); OUT=Path("exp24_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP23_BASE",0),("EXP24A_ENTROPY",1),("EXP24B_TEXT",2),("EXP24C_HYBRID",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir24_{mode}"
    ...
    for name in FILES:
        ...
        # one lossless check per mode/file
...
Path("exp24_adaptive_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
