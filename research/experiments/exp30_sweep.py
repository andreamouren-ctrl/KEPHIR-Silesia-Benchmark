...
ROOT=Path("silesia"); OUT=Path("exp30_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP27A",0),("EXP30A_BLEND50",1),("EXP30B_BLEND75",2),("EXP30C_PURE_PRED",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir30_{mode}"
...
Path("exp30_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
