...
ROOT=Path("silesia"); OUT=Path("exp39_out"); OUT.mkdir(exist_ok=True)
FILES=["dickens","mozilla","mr","nci","ooffice","osdb","reymont","samba","sao","webster","x-ray","xml"]
MODES=[("EXP38A_BASE",0),("EXP39A_TWO_CAND",1),("EXP39B_DELAY_STOP",2),("EXP39C_HYBRID",3)]
rows=[]
for tag,mode in MODES:
    exe=f"./kephir39_{mode}"
    print("\nMODE",tag,flush=True)
    for name in FILES:
...
Path("exp39_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
