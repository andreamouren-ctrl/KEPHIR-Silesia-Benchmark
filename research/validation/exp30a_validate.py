...
ROOT=Path("silesia"); OUT=Path("exp30a_validate"); OUT.mkdir(exist_ok=True)
...
for name in FILES:
    ...
    for _ in range(3):
        ...
        t=time.perf_counter()
        subprocess.run(["./kephir30","cp",str(p),str(arc),"6","6.55","9.42","1.20"],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    ...
    for _ in range(3):
        ...
        t=time.perf_counter()
        subprocess.run(["./kephir30","dp",str(arc),str(dec),"6"],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
summary=dict(profile="KEPHIR EXP-30A POC-BLEND50",raw=raw,size=size,ratio=100*size/raw,
             comp_MBps=(raw/1e6)/ct,dec_MBps=(raw/1e6)/dt,sha_all=True)
print("SUMMARY",summary,flush=True)
for r in rows: print(r,flush=True)
Path("exp30a_validation.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2))
